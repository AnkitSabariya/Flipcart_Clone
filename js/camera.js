...
function local(key){
    ...
    .then((data)=> {
       localStorage.clear()
        ...
        localStorage.setItem("image2",`${data[key].images[1]}`)
        ...
        localStorage.setItem("rating",`${data[key].rating}`)
        localStorage.setItem("stock",`${data[key].stock}`)
        localStorage.setItem("discount",`${data[key].discountPercentage}`)
